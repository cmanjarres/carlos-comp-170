...
namespace IntroCS
{
	class MainClass
	{
		...
		{
			Console.WriteLine("Your grade is {0:F1} %" + LetterGrade, n);
		}
			...
			do{
				...
				if (weightSum == 100) {
					...
					Console.WriteLine("Sorry weights did not add to 100%, please try " +
						"again.\n"); // do we need to concatenate? is this to make it all pretty and uniform?
				}
			}while(weightSum != 100);
			return weightValue;	
		}
		...
		public static double FindAverage(string gradePrompt)
		{
			...
			while (i <= numberOfGrades){
				double score = UI.PromptDouble("Please enter the % grade of " + gradePrompt + " " + i + ": "); //CAM change assignment to exam/hw/project etc
				i++;
				totalScore += score;
			} 
			Console.WriteLine ("Average for {0} is {1}%",gradePrompt,totalScore / numberOfGrades);
